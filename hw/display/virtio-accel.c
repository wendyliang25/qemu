...
struct accel_virgl_resource {
    ...
    MemoryRegion *region;  /* For for host memory mapping */
};
...
static struct accel_virgl_resource *
                                      ...
                                      uint32_t ring_idx, uint64_t fence_id)
{
    ...
    if (!qemu_in_main_thread() && !qemu_mutex_iothread_locked()) {
...
static void accel_cmd_resource_map_blob(VirtIOGPU *g, struct virtio_gpu_ctrl_command *cmd)
{
    ...
    struct accel_virgl_resource *vres;
    MemoryRegion *mr;
    int ret;
    void *data;
    uint64_t size;
